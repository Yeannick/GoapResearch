...
void App_AgarioGame::Start()
{
	...
	for (int i = 0; i < m_AmountOfAgents; i++)
	{
		...
		// Add Decision making stuff
		Elite::Blackboard* pBlackBoard = new Elite::Blackboard();
		pBlackBoard->AddData("Agent", newAgent);
		FiniteStateMachine* pFSM = new FiniteStateMachine(pWanderState,pBlackBoard);

		newAgent->SetDecisionMaking(pFSM); // any kind of decision making structure that inherits from IDecisionmaking
	...
	Elite::Blackboard* pAgentBlackBoard = new Elite::Blackboard();
	pAgentBlackBoard = CreateBlackboard(m_pCustomAgent);
	...
	auto lookForFood = new SeekForFood();
	auto EvadeBiggerAgents = new EvadeState();
	auto PursuitSmallerAgents = new PursuitState();
	auto BorderEvade = new EvadeBorder();
	m_pStates.push_back(lookForFood);
	m_pStates.push_back(EvadeBiggerAgents);
	m_pStates.push_back(PursuitSmallerAgents);
	m_pStates.push_back(BorderEvade);

	//3. Create the transitions beetween those states
	auto TransitionToFood  = new TransitionToSeekFood();
	auto TransitionEvade   = new TransitionToEvade();
	auto TransitionPursuit = new TransitionToPursuit();
	auto TransitiontoWander = new TransitionWander();
	auto TransitionWanderFromEvade = new TransitionToWanderFromEvade();
	auto TransitionWanderFromPursuit = new TransitionToWanderFromPursuit();
	auto TransitionBorder = new TransitionBorderEvade();
	//4. Create the finite state machine with a starting state and the blackboard

	FiniteStateMachine* pFSM = new FiniteStateMachine(pWanderState, pAgentBlackBoard);
	//5. Add the transitions for the states to the state machine

	//from wander to evade and other way around
	pFSM->AddTransition(pWanderState, EvadeBiggerAgents, TransitionEvade);
	pFSM->AddTransition(EvadeBiggerAgents,pWanderState, TransitionWanderFromEvade);

	// from lookforFood to evade ( evade takes priority) and from pursuit to evade for same reason
	pFSM->AddTransition(lookForFood, EvadeBiggerAgents, TransitionEvade);
	pFSM->AddTransition(PursuitSmallerAgents, EvadeBiggerAgents, TransitionEvade);
	//pFSM->AddTransition(BorderEvade, EvadeBiggerAgents, TransitionEvade);
	//from wander to pursuit and other way around
	pFSM->AddTransition(pWanderState, PursuitSmallerAgents, TransitionPursuit);
	pFSM->AddTransition(PursuitSmallerAgents, pWanderState, TransitionPursuit);

	// from wander to food and other way around
	pFSM->AddTransition(pWanderState, lookForFood, TransitionToFood);
	pFSM->AddTransition(lookForFood, pWanderState, TransitiontoWander);

	// from wander to evade border
	pFSM->AddTransition(pWanderState, BorderEvade, TransitionBorder);
	pFSM->AddTransition(BorderEvade, pWanderState, TransitionBorder);
	pFSM->AddTransition(BorderEvade, lookForFood, TransitionBorder);
	pFSM->AddTransition(BorderEvade, PursuitSmallerAgents, TransitionBorder);
